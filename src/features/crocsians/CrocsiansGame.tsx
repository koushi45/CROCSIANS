...
export function CrocsiansGame() {
  ...
  function renderVisitedBaseMap() {
    ...
    return <div ref={isMobileLayout ? undefined : mapViewportRef} className={`${styles.mapViewport} ${styles.socialMapViewport}`} onTouchStart={beginMapTouch} onTouchMove={moveMapTouch} onTouchEnd={finishMapTouch} onTouchCancel={finishMapTouch}>
      <div ref={isMobileLayout ? mapViewportRef : undefined} className={styles.mapCanvas} style={{ width: MAP_PIXEL_SIZE * zoom, height: MAP_PIXEL_SIZE * zoom }}>
        ...
        <div className={styles.baseWalkers}>{walkers.map((person, index) => <BaseWalker key={`${person.ownerId}-${index}`} person={person} index={index} visitor={index > 0} onSelect={person.ownerId !== baseSocialData?.viewerId ? () => setSelectedSocialCharacter(person) : undefined} />)}</div>
      </div>
  ...
  return (
    <main className={`${styles.gameShell} ${textSize === "medium" ? styles.textMedium : textSize === "large" ? styles.textLarge : ""}`} data-crocsians-page>
      ...
      {selectedSocialCharacter && <aside className={styles.characterRelationshipSummary}><h3>{selectedSocialCharacter.name}の関係値</h3>{baseSocialData?.relationships.filter((relation) => relation.fromUserId === selectedSocialCharacter.ownerId || relation.toUserId === selectedSocialCharacter.ownerId).map((relation) => { const counterpartId = relation.fromUserId === selectedSocialCharacter.ownerId ? relation.toUserId : relation.fromUserId; const counterpart = baseSocialData.directory.find((person) => person.ownerId === counterpartId) ?? (baseSocialData.base.ownerId === counterpartId ? baseSocialData.base : null); const status = relation.married ? "配偶者" : relation.romanceActive ? `恋人 ${relation.romanceStage}/10` : relation.rivalryStage > 0 ? `ライバル ${relation.rivalryStage}/10` : relation.friendshipStage > 0 ? `友人 ${relation.friendshipStage}/10` : "知り合い"; return <article key={`${relation.fromUserId}-${relation.toUserId}`}><strong>{counterpart?.name ?? (counterpartId === baseSocialData.viewerId ? characterName : "冒険者")}</strong><em>{status}</em><span>友好 {relation.friendship}・競争 {relation.rivalry}・恋愛 {relation.romance}</span></article>; })}{!baseSocialData?.relationships.some((relation) => relation.fromUserId === selectedSocialCharacter.ownerId || relation.toUserId === selectedSocialCharacter.ownerId) && <p>まだ関係はありません。</p>}</aside>}
