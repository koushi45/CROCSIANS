...
type ExpeditionPanelTab = "status" | "logs";
...
export function CrocsiansGame() {
  ...
  const [expeditionPanelTab, setExpeditionPanelTab] = useState<ExpeditionPanelTab>("status");
            ...
            <div className={styles.playerProfileContent}>
              ...
              <div className={styles.playerProfileDetails}>
                <section><h3>装備</h3><div className={styles.playerEquipment}>
                  ...
                  <article>{inspectedCardinalDefinition ? <NextImage src={inspectedCardinalDefinition.image} alt="" width={96} height={96} unoptimized /> : <span>—</span>}<div><small>CARDINAL</small><strong>{inspectedCardinalDefinition ? inspectedCardinalDefinition.name : "装備なし"}</strong>{inspectedCardinalDefinition && <em>Lv.{inspectedPlayer.cardinalLevels?.[inspectedCardinalDefinition.id] ?? 0}</em>}</div></article>
                </div></section>
        ...
        <aside className={styles.sidePanel}>
          {view === "base" ? (
            ...
            <section className={styles.panelSection}>
              ...
              {view === "explore" && expeditionPanelTab === "status" && <div className={styles.explorationSummary}><div><span>接続プレイヤー</span><b>{mapPlayers.length}人</b></div><div><span>スキル回復まで</span><b>{skillUsesResetAt === null ? "--:--" : formatRecoveryTime(skillRecoveryRemaining)}</b></div></div>}
