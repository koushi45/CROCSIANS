...
type LearnedWord = { id: string; userId: string; category: SocialWordCategoryId; word: string; favorite: boolean; learnedFromUserId: string | null; learnedAt: string };
type TeachPrompt = { character: SocialPerson; category: SocialWordCategoryId; categoryLabel: string; question: string; example: string };
...
type BaseSocialResponse = { viewerId: string; directory: SocialBaseSummary[]; base: SocialBaseApi; relationships: SocialRelationship[]; learnedWords: LearnedWord[]; conversationLogs: SocialConversation[]; selfWordTeachRemaining: number; selfTeachExamples: Partial<Record<SocialWordCategoryId, string>>; error?: string };
...
function HighlightedWord({ text, word }: { text: string; word: string }) {
  if (!word) return text;
  return <>{text.split(word).map((part, index, parts) => <span key={`${index}-${part}`}>{part}{index < parts.length - 1 && <strong className={styles.highlightedSocialWord}>{word}</strong>}</span>)}</>;
}
...
export function CrocsiansGame() {
  ...
  const [characterModalTab, setCharacterModalTab] = useState<"logs" | "relations" | "words">("logs");
  const [conversationLogPeriod, setConversationLogPeriod] = useState<"day" | "week" | "month" | "all">("day");
  const [selfTeachCategory, setSelfTeachCategory] = useState<SocialWordCategoryId>("happy");
  const [selfTeachWord, setSelfTeachWord] = useState("");
  const [selfTeachSubmitting, setSelfTeachSubmitting] = useState(false);
  ...
  useEffect(() => {
    if (!gameStarted || view !== "base" || !baseSocialData?.directory.length) return;
    ...
    const perform = async () => {
      ...
      const residents = visitedBase ? [viewer, visitedBase, ...visitedBase.visitors] : [viewer, ...baseSocialData.directory];
      const firstIndex = Math.floor(Math.random() * residents.length);
      let secondIndex = Math.floor(Math.random() * (residents.length - 1));
      if (secondIndex >= firstIndex) secondIndex += 1;
      let actor = residents[firstIndex];
      let partner = residents[secondIndex];
      // APIの対象には閲覧者以外を渡し、どの拠点にいても同じ交流処理を使う。
      if (partner.ownerId === baseSocialData.viewerId) [actor, partner] = [partner, actor];
  ...
  useEffect(() => {
    if (!gameStarted || view !== "base") return;
    let active = true;
    let timer = 0;
  ...
  async function submitSelfTaughtWord(event: FormEvent) {
    event.preventDefault();
    if (!selfTeachWord.trim() || selfTeachSubmitting || !baseSocialData?.selfWordTeachRemaining) return;
    setSelfTeachSubmitting(true);
    try {
      const response = await fetch("/api/crocsians/base-social", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action: "selfTeachWord", category: selfTeachCategory, word: selfTeachWord }) });
      const result = await response.json() as { error?: string; message?: string; remaining?: number };
      if (!response.ok) throw new Error(result.error ?? "言葉を覚えられませんでした");
      setSystemMessage(result.message ?? `「${selfTeachWord.trim()}」を覚えました`);
      setSelfTeachWord("");
      await loadBaseSocial(visitedBase?.ownerId);
    } catch (error) {
      setSystemMessage(error instanceof Error ? error.message : "言葉を覚えられませんでした");
    } finally {
      setSelfTeachSubmitting(false);
    }
  }
  ...
  function renderVisitedBaseMap() {
    ...
    return <div ref={isMobileLayout ? undefined : mapViewportRef} className={`${styles.mapViewport} ${styles.socialMapViewport}`} onTouchStart={beginMapTouch} onTouchMove={moveMapTouch} onTouchEnd={finishMapTouch} onTouchCancel={finishMapTouch}>
      <div ref={isMobileLayout ? mapViewportRef : undefined} className={styles.mapCanvas} style={{ width: MAP_PIXEL_SIZE * zoom, height: MAP_PIXEL_SIZE * zoom }}>
        ...
        <div className={styles.baseWalkers}>{walkers.map((person, index) => <BaseWalker key={`${person.ownerId}-${index}`} person={person} index={index} visitor={index > 0} onSelect={() => { setCharacterModalTab("logs"); setSelectedSocialCharacter(person); }} />)}</div>
      </div>
    ...
    <main className={`${styles.gameShell} ${textSize === "medium" ? styles.textMedium : textSize === "large" ? styles.textLarge : ""}`} data-crocsians-page>
      ...
      <div className={styles.statusHud} aria-label="プレイヤーステータス">
        <div className={styles.statusHudCore}>
          ...
          <span><small>SKILL RECOVERY</small><b>{skillUsesResetAt === null ? "--:--" : formatRecoveryTime(skillRecoveryRemaining)}</b></span>
        </div>
      </div>
        ...
        <section className={`${styles.mainStage} ${view === "base" ? styles.baseStage : ""} ${view === "explore" ? styles.exploringStage : ""}`}>
          <div className={styles.stageTitle}>
            ...
            <div className={styles.stageMeta}>
              {view === "base" && <div className={styles.mapTools}><button className={styles.socialHubButton} type="button" onClick={() => { setBaseSocialOpen(true); void loadBaseSocial(); }}>拠点交流</button><button className={styles.socialHubButton} type="button" onClick={() => { setCharacterModalTab("logs"); setSelectedSocialCharacter({ ownerId: baseSocialData?.viewerId ?? "local", name: characterName, job, level: playerProgress.level, icon: characterIcon }); }}>自分の会話ログ</button><span>表示 <b>{Math.round(zoom * 100)}%</b></span><span>建物 <b>{Object.keys(visitedBase?.buildings ?? buildings).length}/{MAP_CELL_COUNT / (FACILITY_SIZE * FACILITY_SIZE)}</b></span><button title="縮小" onClick={() => changeMapZoom(zoom - .1)}>−</button><button title="拡大" onClick={() => changeMapZoom(zoom + .1)}>＋</button></div>}
          ...
          {view === "base" && (visitedBase ? renderVisitedBaseMap() : (
            <div ref={isMobileLayout ? undefined : mapViewportRef} className={styles.mapViewport} onTouchStart={beginMapTouch} onTouchMove={moveMapTouch} onTouchEnd={finishMapTouch} onTouchCancel={finishMapTouch}>
              <div ref={isMobileLayout ? mapViewportRef : undefined} className={styles.mapCanvas} style={{ width: MAP_PIXEL_SIZE * zoom, height: MAP_PIXEL_SIZE * zoom }}>
                ...
                <div className={styles.baseWalkers}><BaseWalker person={socialScene?.actor ?? { ownerId: baseSocialData?.viewerId ?? "local", name: characterName, job, level: playerProgress.level, icon: characterIcon }} index={0} visitor={Boolean(socialScene && socialScene.actor.ownerId !== baseSocialData?.viewerId)} onSelect={() => { setCharacterModalTab("logs"); setSelectedSocialCharacter(socialScene?.actor ?? { ownerId: baseSocialData?.viewerId ?? "local", name: characterName, job, level: playerProgress.level, icon: characterIcon }); }} pairSide={socialScene ? "left" : undefined} speech={actorSocialSpeech} activity={socialScene?.relationshipEvent?.eventType === "wedding" ? "wedding" : socialScene?.relationshipEvent?.relationshipKind === "romance" ? "romanceTalk" : socialScene?.relationshipEvent?.relationshipKind === "rivalry" ? "rivalTalk" : socialScene?.relationshipEvent?.relationshipKind === "friendship" ? "friendTalk" : socialScene?.activity} />{socialScene && <BaseWalker key={socialScene.partner.ownerId} person={socialScene.partner} index={1} visitor={socialScene.partner.ownerId !== baseSocialData?.viewerId} onSelect={() => { setCharacterModalTab("logs"); setSelectedSocialCharacter(socialScene.partner); }} pairSide="right" speech={partnerSocialSpeech} activity={socialScene.relationshipEvent?.eventType === "wedding" ? "wedding" : socialScene.relationshipEvent?.relationshipKind === "romance" ? "romanceTalk" : socialScene.relationshipEvent?.relationshipKind === "rivalry" ? "rivalTalk" : socialScene.relationshipEvent?.relationshipKind === "friendship" ? "friendTalk" : socialScene.activity === "sing" ? "listenSong" : socialScene.activity === "speech" ? "listenSpeech" : socialScene.activity} />}{baseSocialData && baseSocialData.base.ownerId === baseSocialData.viewerId && baseSocialData.base.visitors.filter((person) => person.ownerId !== socialScene?.partner.ownerId && person.ownerId !== socialScene?.actor.ownerId).slice(0, socialScene?.relationshipEvent?.eventType === "wedding" ? 12 : 4).map((person, index) => <BaseWalker key={person.ownerId} person={person} index={index + 2} visitor onSelect={() => { setCharacterModalTab("logs"); setSelectedSocialCharacter(person); }} activity={socialScene?.relationshipEvent?.eventType === "wedding" ? "wedding" : undefined} />)}</div>
              </div>
      ...
      {selectedSocialCharacter && (() => { const cutoff = conversationLogPeriod === "all" ? 0 : Date.now() - (conversationLogPeriod === "day" ? 1 : conversationLogPeriod === "week" ? 7 : 30) * 86_400_000; const logs = baseSocialData?.conversationLogs.filter((log) => (log.speakerId === selectedSocialCharacter.ownerId || log.listenerId === selectedSocialCharacter.ownerId) && new Date(log.createdAt).getTime() >= cutoff) ?? []; const relations = baseSocialData?.relationships.filter((relation) => relation.fromUserId === selectedSocialCharacter.ownerId || relation.toUserId === selectedSocialCharacter.ownerId) ?? []; const words = baseSocialData?.learnedWords.filter((word) => word.userId === selectedSocialCharacter.ownerId) ?? []; const isMine = selectedSocialCharacter.ownerId === baseSocialData?.viewerId; return <div className={styles.catalogBackdrop} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelectedSocialCharacter(null); }}><section className={styles.characterConversationDialog} role="dialog" aria-modal="true"><header><span>{selectedSocialCharacter.icon ? <NextImage src={selectedSocialCharacter.icon} alt="" width={128} height={128} unoptimized /> : selectedSocialCharacter.name.charAt(0)}</span><div><small>CHARACTER MEMORIES</small><h2>{selectedSocialCharacter.name}</h2></div><button type="button" onClick={() => setSelectedSocialCharacter(null)}>×</button></header><nav className={styles.characterMemoryTabs}><button type="button" className={characterModalTab === "logs" ? styles.characterMemoryTabActive : ""} onClick={() => setCharacterModalTab("logs")}>会話ログ</button><button type="button" className={characterModalTab === "relations" ? styles.characterMemoryTabActive : ""} onClick={() => setCharacterModalTab("relations")}>関係値</button><button type="button" className={characterModalTab === "words" ? styles.characterMemoryTabActive : ""} onClick={() => setCharacterModalTab("words")}>覚えた言葉</button></nav><div className={styles.characterConversationBody}>{characterModalTab === "logs" && <><div className={styles.conversationPeriodPicker}><span>表示期間</span>{([["day", "24時間"], ["week", "一週間"], ["month", "一カ月"], ["all", "すべて"]] as const).map(([value, label]) => <button type="button" key={value} className={conversationLogPeriod === value ? styles.characterMemoryTabActive : ""} onClick={() => setConversationLogPeriod(value)}>{label}</button>)}</div>{logs.map((log) => <article key={log.id}><time>{new Date(log.createdAt).toLocaleString("ja-JP")}</time><strong>{SOCIAL_WORD_CATEGORY_MAP[log.category]?.label ?? log.category}</strong><p><HighlightedWord text={log.message} word={log.word} /></p><small>話題の言葉「<HighlightedWord text={log.word} word={log.word} />」</small></article>)}{!logs.length && <p className={styles.catalogEmpty}>選択した期間の会話ログはありません。</p>}</>}{characterModalTab === "relations" && <div className={styles.characterRelationshipList}>{relations.map((relation) => { const counterpartId = relation.fromUserId === selectedSocialCharacter.ownerId ? relation.toUserId : relation.fromUserId; const counterpart = baseSocialData?.directory.find((person) => person.ownerId === counterpartId) ?? (baseSocialData?.base.ownerId === counterpartId ? baseSocialData?.base ?? null : null); const status = relation.married ? "配偶者" : relation.romanceActive ? `恋人 ${relation.romanceStage}/10` : relation.rivalryStage > 0 ? `ライバル ${relation.rivalryStage}/10` : relation.friendshipStage > 0 ? `友人 ${relation.friendshipStage}/10` : "知り合い"; return <article key={`${relation.fromUserId}-${relation.toUserId}`}><strong>{counterpart?.name ?? (counterpartId === baseSocialData?.viewerId ? characterName : "冒険者")}</strong><em>{status}</em><span>友好 {relation.friendship}・競争 {relation.rivalry}・恋愛 {relation.romance}</span><small>交流 {relation.interactionCount}回</small></article>; })}{!relations.length && <p className={styles.catalogEmpty}>まだ関係はありません。</p>}</div>}{characterModalTab === "words" && <><form className={styles.selfTeachWordForm} onSubmit={submitSelfTaughtWord}><header><strong>新しい言葉を覚える</strong><span>本日あと {baseSocialData?.selfWordTeachRemaining ?? 0}/3回</span></header><label><span>カテゴリー</span><select value={selfTeachCategory} onChange={(event) => setSelfTeachCategory(event.target.value as SocialWordCategoryId)}>{Object.entries(SOCIAL_WORD_CATEGORY_MAP).map(([id, definition]) => <option key={id} value={id}>{definition.label}</option>)}</select></label><label><span>覚える言葉</span><input value={selfTeachWord} maxLength={40} placeholder="言葉をひとつ入力" onChange={(event) => setSelfTeachWord(event.target.value)} /></label><section><small>会話での使用例（相手：B）</small><p><HighlightedWord text={(baseSocialData?.selfTeachExamples[selfTeachCategory] ?? `{A}「{B}、『{word}』について話したいんだ」`).replaceAll("{A}", selectedSocialCharacter.name).replaceAll("{B}", "B").replaceAll("{word}", selfTeachWord.trim() || "{word}")} word={selfTeachWord.trim() || "{word}"} /></p></section><button type="submit" disabled={!selfTeachWord.trim() || selfTeachSubmitting || !baseSocialData?.selfWordTeachRemaining}>{selfTeachSubmitting ? "覚えています…" : baseSocialData?.selfWordTeachRemaining ? "この言葉を覚える" : "本日の上限に達しました"}</button></form><section className={styles.favoriteWordManager}>{words.map((word) => isMine ? <button type="button" key={word.id} className={word.favorite ? styles.favoriteWordActive : ""} onClick={() => void toggleFavoriteWord(word)}><span>{word.favorite ? "★" : "☆"} {word.word}</span><small>{SOCIAL_WORD_CATEGORY_MAP[word.category]?.label}</small></button> : <article key={word.id}><span>{word.word}</span><small>{SOCIAL_WORD_CATEGORY_MAP[word.category]?.label}</small></article>)}{!words.length && <p>まだ言葉を覚えていません。</p>}</section></>}</div></section></div>; })()}
      ...
      {mobileBuildCell !== null && (() => {
        ...
        const isTileCategory = constructionCategory === "tile";
        const categoryLabel: Record<ConstructionCategory, string> = { production: "生産施設", workshop: "工房", decoration: "景観施設", tile: "床・タイル" };
